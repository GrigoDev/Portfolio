...
// distance возвращает дистанцию в км., которую преодолел пользовательи за тренировку.
...
func (t Training) meanSpeed() float64 {
	// добавил проверку на нулевую продолжительность тренировки.
...
// пока 0, так как этот метод будет переопределяться для каждого типа тренировки.
...
func (w Walking) Calories() float64 {
	// добавил проверку на нулевую продолжительность тренировки и на нулевой рост пользователя.
	if w.Duration.Hours() == 0 || w.Height == 0 {
		return 0
	}
	heightMeters := w.Height / CmInM             // рост в метрах
	meanSpeedMeters := w.meanSpeed() * KmHInMsec // средняя скорость в м/с
	return ((CaloriesWeightMultiplier*w.Weight + math.Pow(meanSpeedMeters, 2)/(heightMeters)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func (s Swimming) TrainingInfo() InfoMessage {
	// добавил расчет дистанции тренировки в басейне исходя из его длины и кол-ва его пересечений.
	//distance := float64(s.LengthPool * s.CountPool) / MInKm
	// так же добавил проверку на 0 в значениях длины басейна и кол-ве его пересечений
	//if s.LengthPool == 0 || s.CountPool == 0 {
	//	distance = 0
	//}

	// переопределил метод TrainingInfo для структуры Swimming.
	return InfoMessage{
...
func ReadData(training CaloriesCalculator) string {
	...
	// добавляю полученные калории в структуру с информацией о тренировке
